...
export default function CogniLexAI() {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans relative">
      ...
      {/* SIDEBAR */}
      <aside className={`${isSidebarOpen ? "w-80" : "w-0"} bg-slate-900 border-r border-slate-800 transition-all duration-500 flex flex-col`}>
        <div className="p-6 flex-1 overflow-y-auto no-scrollbar">
          <div className="flex items-center gap-3 mb-10">
            <div className="w-8 h-8 bg-amber-600 rounded-lg flex items-center justify-center font-bold text-white shadow-[0_0_20px_rgba(217,119,6,0.4)]">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /><path d="M2 12l10 5 10-5" />
              </svg>
            </div>
            <h1 className="text-xl font-bold tracking-tight bg-gradient-to-r from-amber-50 to-amber-200 bg-clip-text text-transparent">CogniLex AI</h1>
          </div>

          <button onClick={() => { setMessages([]); setActiveTab("chat"); }} className="w-full flex items-center justify-center gap-2 py-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition mb-6 text-sm font-medium">
          ...
          <nav className="space-y-3">
            ...
            <div onClick={() => setActiveTab("chat")} className={`p-3 rounded-xl flex items-center gap-3 cursor-pointer transition ${activeTab === 'chat' ? 'bg-amber-600/10 border border-amber-500/20 text-amber-500' : 'hover:bg-white/5 border border-transparent'}`}>
            ...
            <div className="pt-6 mt-6 border-t border-slate-800">
              ...
              <div
                ...
                className="p-3 hover:bg-white/5 rounded-xl flex items-center gap-3 cursor-pointer transition group border border-transparent hover:border-amber-500/30 mb-3"
              >
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50 relative">
          {showUserMenu && (
            <div className="absolute bottom-full left-4 right-4 mb-2 bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in slide-in-from-bottom-2 duration-200">
              <div className="p-3 flex items-center gap-3 border-b border-slate-700">
                <div className="w-8 h-8 bg-slate-900 rounded-full flex items-center justify-center text-[10px] text-amber-500 border border-slate-700 font-bold">
              ...
              <div className="p-1.5 border-t border-slate-700">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2 hover:bg-rose-500/10 rounded-lg transition text-[11px] text-rose-400 mt-1"
                >
          ...
          <div
            ...
            className="flex items-center gap-3 px-2 py-1.5 cursor-pointer hover:bg-slate-800 rounded-xl transition"
          >
            <div className="relative">
              <div className="w-9 h-9 bg-slate-900 rounded-full flex items-center justify-center text-amber-500 font-bold text-xs shadow-lg border border-slate-700">
                {getUserInitials(currentUser?.name, currentUser?.email)}
              </div>
      ...
      {/* MAIN AREA */}
      <main className="flex-1 flex flex-col relative bg-gradient-to-b from-slate-900 to-slate-950">
        ...
        <header className="h-16 flex items-center justify-between px-8 border-b border-slate-800/50 backdrop-blur-md bg-slate-900/40 z-10">
          <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="p-2 hover:bg-slate-800 rounded-lg transition text-slate-400 font-bold text-xl">☰</button>
        ...
        {activeTab === "chat" ? (
          <>
            <div className="flex-1 overflow-y-auto px-6 md:px-24 py-10 space-y-8 no-scrollbar">
              {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center animate-in fade-in duration-700">
                  <div className="w-20 h-20 bg-slate-800 rounded-3xl flex items-center justify-center text-3xl mb-6 border border-slate-700 shadow-[0_0_50px_rgba(217,119,6,0.05)] text-amber-500">⚖️</div>
                  <h2 className="text-3xl font-black text-white mb-2 tracking-tight">CogniLex Legal Assistant</h2>
                  <p className="text-slate-400 text-sm max-w-md text-center">Secure AI intelligence tailored for Sri Lankan jurisprudence. Formulate your inquiry below.</p>
                </div>
              )}
              {messages.map((msg, i) => (
                <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2`}>
                  <div className={`max-w-[85%] p-5 rounded-[2rem] ${msg.role === 'user' ? 'bg-slate-800 border border-slate-700 text-white rounded-br-none shadow-xl shadow-slate-900/20' : 'bg-slate-900 text-slate-200 border border-slate-800 rounded-bl-none shadow-2xl'}`}>
            ...
            <div className="px-6 md:px-32 pb-8 pt-4">
              <div className="bg-slate-900 border border-slate-700 rounded-full p-2 flex items-center gap-2 focus-within:border-amber-500/50 shadow-2xl transition-all duration-300">
                <button onClick={startListening} className="p-3 text-slate-400 hover:text-amber-500 transition-all hover:bg-slate-800 rounded-full">
                ...
                <input className="flex-1 bg-transparent border-none outline-none py-3 px-2 text-sm text-white placeholder-slate-500" placeholder="State your legal inquiry or scenario..." value={question} onChange={(e) => setQuestion(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} />
          ...
          <div className="flex-1 overflow-y-auto p-10 md:px-20 animate-in fade-in slide-in-from-right-5 duration-500 text-center flex flex-col items-center justify-center">
            <h2 className="text-3xl font-black text-white mb-2 tracking-tight">System Analytics</h2>
            ...
            <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 w-full">
              <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-xl flex flex-col items-center">
                ...
                <p className="text-4xl font-black text-white">{stats?.total_queries || 0}</p>
              </div>
              <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-xl flex flex-col items-center">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Active Users</p>
                <p className="text-4xl font-black text-white">{stats?.active_users || 0}</p>
              </div>
            </div>
            <button onClick={() => setActiveTab("chat")} className="bg-slate-800 border border-slate-700 px-8 py-3 rounded-full hover:bg-slate-700 transition text-sm font-bold text-white shadow-md">Return to Session</button>
          </div>
        )}
      </main>
    </div>
