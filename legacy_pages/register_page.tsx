...
export default function RegisterPage() {
    ...
    const [loadingProvider, setLoadingProvider] = useState<'sendOtp' | 'verifyOtp' | 'resendOtp' | 'google' | 'microsoft' | null>(null);
            ...
            <div className="max-w-5xl w-full bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden flex flex-col md:flex-row min-h-[600px]">
                ...
                <div className="w-full md:w-7/12 p-8 md:p-12 relative flex flex-col justify-center">
                    ...
                    <form className="space-y-5" onSubmit={isOtpStep ? handleVerifyOtp : handleSendOtp}>
                        ...
                        {isOtpStep && (
                            <div>
                                ...
                                <button
                                    ...
                                    disabled={isLoading}
                                    className="mt-2 text-sm font-bold text-amber-600 hover:text-amber-700 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer flex items-center gap-1.5"
                                >
                                    {loadingProvider === 'resendOtp' ? (
                                        <>
                        ...
                        >
                            {(loadingProvider === 'sendOtp' || loadingProvider === 'verifyOtp') ? (
                                <div className="animate-spin rounded-full h-5 w-5 border-2 border-white border-t-transparent"></div>
                            ) : (
                                <UserPlus className="w-5 h-5" />
                            )}
                            <span>
                                {loadingProvider === 'sendOtp' ? 'Sending OTP...' :
                                 loadingProvider === 'verifyOtp' ? 'Verifying OTP...' :
                                 isOtpStep ? 'Verify & Register' : 'Send OTP'}
                            </span>
                        </button>
                    </form>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <button
                            ...
                            disabled={isLoading}
                            className="flex items-center justify-center gap-2 py-3 bg-white border border-slate-300 shadow-sm rounded-xl hover:bg-slate-50 transition cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
                        >
                            ...
                            <span className="font-bold text-slate-700 text-sm">
                                {loadingProvider === 'google' ? 'Signing up...' : 'Google'}
                            </span>
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            <span className="font-bold text-slate-700 text-sm">
                                {loadingProvider === 'microsoft' ? 'Signing up...' : 'Microsoft'}
                            </span>
                        </button>
                    </div>
