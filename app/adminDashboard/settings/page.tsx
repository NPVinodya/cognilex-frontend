...
export default function AdminSettingsPage() {
  ...
  const toggleDarkMode = async () => {
    ...
    try {
      ...
      if (adminStr) {
        const adminId = JSON.parse(adminStr).id;
        await fetch(`http://localhost:8000/admin/preferences/${adminId}`, {
          method: 'PUT',
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      const adminStr = localStorage.getItem('adminUser');
      const adminId = adminStr ? JSON.parse(adminStr).id : null;
      const response = await fetch(`http://localhost:8000/admin/change-password/${adminId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3">
          {status.message && (
            <div className={`mb-6 p-4 rounded-2xl border animate-in slide-in-from-top duration-300 ${
              status.type === 'success' ? 'bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30 text-red-700 dark:text-red-400'
            }`}>
