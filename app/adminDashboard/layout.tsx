...
import { 
  Users, Scale, Shield, Bell, Menu, X, LayoutDashboard, Clock, LogOut
} from 'lucide-react';
...
export default function AdminDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FA] font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-[#181B25] z-[101] lg:hidden transform transition-transform duration-300 ease-in-out shadow-2xl flex flex-col ${
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 md:px-10 z-10 shrink-0">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 -ml-2 lg:hidden text-slate-500 hover:bg-slate-100 rounded-lg transition">
              <Menu className="h-6 w-6" />
            </button>
            <div className="hidden md:flex items-center gap-2">
              <Shield className="h-5 w-5 text-[#FF9000]" />
              <span className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em]">Platform Administration</span>
            </div>
          </div>

          <div className="flex items-center gap-5">
            <button className="relative p-2.5 text-slate-500 hover:text-[#FF9000] hover:bg-orange-50 rounded-xl transition-all duration-300">
              <Bell className="h-5 w-5" />
              <span className="absolute top-2.5 right-2.5 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white"></span>
            </button>
            <div className="h-10 w-10 rounded-xl bg-[#181B25] border border-slate-200 flex items-center justify-center text-[#FF9000] font-black shadow-sm overflow-hidden">
