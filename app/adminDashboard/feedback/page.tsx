...
export default function FeedbackPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const response = await fetch(`http://localhost:8000/admin/feedback/${id}/status?status_value=${status}`, {
        method: 'PATCH',
      });
      if (response.ok) {
  ...
  const deleteFeedback = async (id: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/admin/feedback/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
